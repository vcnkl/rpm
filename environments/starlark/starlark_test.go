...
rpm_target(ref = "api:serve", command = "go run .", workdir = "/repo/api", readiness_cmd = "test -f /tmp/api-ready", env = {"A": "b"}, reload = True)
...
`)
	...
	assert.Equal(t, "test -f /tmp/api-ready", plan.Targets[0].ReadinessCmd)
